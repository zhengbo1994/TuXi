                    ...
                    '<div class="eleTree-node-content" style="padding-left: '+(options.indent*floor)+'px;">',
                        ...
                        function() {
                            ...
                            return ''
                                 ...
                                 '<span class="eleTree-node-content-label" title="{{d[i]["' + options.request.name + '"]}}">{{d[i]["' + options.request.name + '"].length>9?(d[i]["' + options.request.name + '"].substring(0,9)+"..."):d[i]["' + options.request.name + '"]}}</span>',
                    '</div>',
        ...
        rightClickMenu: function() {
            ...
            options.elem.off("contextmenu").on("contextmenu",".eleTree-node-content",function(e) {
                ...
                $("#tree-menu li.add").off().on("click",function(e) {
                    ...
                    var obj = {
                        id: $("div.eleTree-node-content-active").parent().attr("data-id"),
                        label: $("div.eleTree-node-content-active").children(":eq(1)").text()
                    }
                    options.addCallback(obj);
                })
                // 添加的默认数据
                var obj={};
