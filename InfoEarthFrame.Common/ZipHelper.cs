...
namespace InfoEarthFrame.Common
{
    public class ZipHelper
    {
        ...
        public ZipHelper(string sourceFolderPath)
        {
        ...
        public bool ZipDir(string dirToZip, string zipedFileName, int compressionLevel = 9)
        {
            try
            {
                ...
                using (var zipoutstream = new ZipOutputStream(File.Create(zipedFileName)))
                {
                    zipoutstream.SetLevel(compressionLevel);
                    Crc32 crc = new Crc32();
                    Hashtable fileList = GetAllFies(dirToZip);

                    int unit = 10 * 1024 * 1024;//按照每10MB写入压缩文件
                    foreach (DictionaryEntry item in fileList)
                    {
                        ...
                        {
                            byte[] buffer1 = new byte[unit];
                            ZipEntry entry1 = new ZipEntry(Path.GetFileName(item.Key.ToString()))
                            {
                                DateTime = (DateTime)item.Value,
                                Size = fs.Length
                            };
                            ...
                            {
                                if ((fs.Position + unit) > fs.Length)
                                {
                                    buffer1 = new byte[fs.Length - fs.Position];
                                }
                                fs.Read(buffer1, 0, buffer1.Length);
                                zipoutstream.Write(buffer1, 0, buffer1.Length);
                            }
                            fs.Close();
                        }
                    }
