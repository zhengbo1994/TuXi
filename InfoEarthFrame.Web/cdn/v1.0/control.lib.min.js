...
var a=params[key],b="",icon="",tip="",bgclr="",stylestr="";if(a.icon){icon='<i class="'+a.icon+'"></i>';}
if(a.bgclr){bgclr=' ng-style="{\'background-color\':\'rgb(\'+'+this._opt.row+'.'+a.bgclr+'+\')\'}" ';}
if(a.stylestr){stylestr=a.stylestr;}
if(a.tip){tip='tooltip="'+a.tip+'" tooltip-append-to-body';}
var strs=this._opt.row+'.isShowInput['+i+']';var clickstr='ng-click="'+strs+'=true;'+this._opt.cellFunName+'('+this._opt.row+',\''+key+'\',$event,true)"';var strss='<{{type}}  '+bgclr+tip+' ng-class="{\'input-display\':'+strs+'}"  style="margin:0px 2px;'+stylestr+'" '+clickstr+'>'+icon+'{{'+this._opt.row+'.'+key+'}}</{{type}}>';var editstr='<input class="form-control" type="text" ng-model="'+this._opt.row+'.'+key+'" ng-blur="'+strs+'=false" ng-class="{\'input-display\':!'+strs+'}" ng-keyup="keydownTableInput('+this._opt.row+', $event,'+i+')" />'+strss;if(a.iseditnum){a.isedit=true;editstr='<num-box text-model="'+this._opt.row+'.'+key+'" ng-blur="'+strs+'=false" ng-class="{\'input-display\':!'+strs+'}" ng-keyup="keydownTableInput('+this._opt.row+', $event,'+i+')" ></num-box>'+strss;}
b+=a.isclick?a.isedit?editstr:'<{{type}} '+bgclr+tip+' style="margin:0px 2px;'+stylestr+'" ng-click="'+this._opt.cellFunName+'('+this._opt.row+',\''+key+'\',$event)">'+icon+'{{'+this._opt.row+'.'+key+'}}</{{type}}>':'<{{type}} '+bgclr+' style="margin:0px 2px;'+stylestr+'">{{'+this._opt.row+'.'+key+'}}</{{type}}>';b=b.replace(/{{type}}/g,a.type);html+='<td '+(a.width&&i===1?'style="width:'+a.width+';" ':'')+(a.isedit?clickstr:'')+'ng-class="{\'td-paddingSX0\':'+strs+'}"><div style="text-align:left;margin:0;white-space:nowrap;word-wrap:normal;overflow:hidden;display:block;text-overflow:ellipsis;">'+b+'</div></td>';i++;}}
...
i=32*i;if(i===32){i=65;}
i=i+'px';html='<td style="width:'+i+' ;"><div style="width:'+i+';text-align:left;margin:0;white-space:nowrap;word-wrap:normal;overflow:hidden;display:block;text-overflow:ellipsis;">'+html+'</div></td>';thead='</div></th><th><div width="true" style="width:'+i+';min-width:55px;text-align:left;margin:0;white-space:nowrap;word-wrap:normal;overflow:hidden;display:block;text-overflow:ellipsis;" translate="setting.tableOperate">操作';}
...
return tep;},replace:true,scope:{popupElement:"=?",dataset:"=?",mapheight:"=?",measuremap:'=?',clearDraw:'=?',getExtent:'=?',addTagging:'=?',addChart:'=?',addMarker:'=?',addLocation:'=?',map:'=?',addLayer:'=?',removeLayer:'=?',isShowTools:'=?',isLine:'=?',isArea:'=?',isReset:'=?',isClear:'=?',isFullscreen:'=?',isZoominorout:'=?',isLoadInternetTianDiTu:'=?',onDrawstart:'=?',onDrawend:'=?',},link:function(scope,element,attr){var map,vector=new ol.layer.Vector(),topName,chkId,select,dragBox,selectedFeatures,MeasureTool,isAgain=false,vectorLayer,lyr={},typeSelect={},vectorSource,locationLayer;var baseMap_map=scope.dataset?scope.dataset.baseMap.map:{};var baseMap_note=scope.dataset?scope.dataset.baseMap.note:{};var statellite_map=scope.dataset?scope.dataset.statelliteMap.map:{};var statellite_note=scope.dataset?scope.dataset.statelliteMap.note:{};var mapheight=angular.isUndefined(scope.mapheight)?550:scope.mapheight;scope.addLayer=function(layer,bounds){if(map===undefined){return scope.addLayer(layer,bounds);}
...
zjLayer.setZIndex(18);map.addLayer(zjLayer);changeColor(0,1);initMap();scope.map=map;};if(!scope.dataset){layerManager.getAllList().success(function(data){if(!data){return;}
...
if(parseInt(scope.notiModel)<1){scope.showModel="";}}})
elem.children("a").bind("click",function(){if(scope.isHidden){$timeout(function(){scope.isShowNum=false;scope.notiModel='';scope.showModel='';},0);}});},replace:false}});app.directive('notifiRow',function($timeout){return{restrict:'E',template:function(){var templates='<div class="list-group-item notification-list">'+'<a href="javascript:;" ng-click="onNotiClick()">'+'{{message}} '+'<span class="badge">{{notiModel}}</span>'+'</a>'+'</div>';return templates;},scope:{notiModel:'=?',onNotiClick:'=?',message:'=?',isHidden:'=?'},link:function(scope,elem,attr,ctrl){scope.notiModel=angular.isUndefined(scope.notiModel)?'':scope.notiModel;scope.isHidden=angular.isUndefined(scope.isHidden)?true:scope.isHidden;elem.children().children("a").bind("click",function(){if(scope.isHidden){$timeout(function(){scope.notiModel='';},0);}});},replace:false}});;'use strict';app.directive('numBox',function($timeout){return{restrict:'E',template:function(elem,attr){var templates="";var width=attr.width?attr.width:60;if(!angular.isUndefined(attr.textTitle)){templates='<label class="control-label" style="display: table-cell; vertical-align: middle; padding-right: 30px">'+'{{textTitle}}'+'</label>';}
templates='<div class="input-group bootstrap-touchspin">'+
templates+'<div style="position:relative;">'+'<input type="text" class="form-control" placeholder="{{textPlaceholder}}" style="width:'+width+'px" '+'ng-model="textModel" '+' >'+'<a class="input-cancel" href="javascript:;"><i class="fa fa-times" style="color: gray" ng-click="cleanInput()"></i></a>'+'</div>'+'<span class="input-group-btn-vertical" style="width:auto" ng-show="showButton">'+'<a class="btn btn-default bootstrap-touchspin-up no-radius" ng-click="numup()">'+'<i class="glyphicon glyphicon-chevron-up"></i>'+'</a>'+'<a class="btn btn-default bootstrap-touchspin-down no-radius" ng-click="numdown()">'+'<i class="glyphicon glyphicon-chevron-down"></i>'+'</a>'+'</span>'+'</div>';return templates;},scope:{maxNum:'=?',minNum:'=?',textModel:'=?',showButton:'=?',step:'=?',precision:'=?'},link:function(scope,elem,attr){scope.textModel=angular.isUndefined(scope.textModel)?0:scope.textModel;scope.showButton=angular.isUndefined(scope.showButton)?false:scope.showButton;scope.step=angular.isUndefined(scope.step)?1:scope.step;scope.precision=angular.isUndefined(scope.precision)?-1:parseInt(scope.precision);if(!angular.isUndefined(attr.textTitle)){if(attr.textTitle===""||attr.textTitle===null){scope.textTitle="请输入：";}
else{scope.textTitle=attr.textTitle;}}
scope.textPlaceholder=angular.isUndefined(attr.textPlaceholder)?'请输入数字':attr.textPlaceholder;scope.maxNum=angular.isUndefined(scope.maxNum)?"max":scope.maxNum;scope.minNum=angular.isUndefined(scope.maxNum)?"max":scope.minNum;if(scope.maxNum==""||scope.maxNum==null){}
if(scope.minNum==""||scope.minNum==null){}
...
else{event.preventDefault();}}});elem.bind('keyup',function(event){var key=event.keyCode?event.keyCode:event.which;if(key===8||key===46){return;}
$timeout(function(){if(scope.maxNum!=="max"){if(parseFloat(scope.textModel)>parseFloat(scope.maxNum)){scope.textModel=parseFloat(scope.maxNum);}}
if(scope.minNum!=="min"){if(parseFloat(scope.textModel)<parseFloat(scope.minNum)){scope.textModel=parseFloat(scope.minNum);}}},0);});elem.children("div").children("input").bind('blur',function(){if(scope.textModel||scope.textModel===0){$timeout(function(){scope.textModel=changetemp(scope.textModel);if(scope.maxNum!=="max"){if(parseFloat(scope.textModel)>parseFloat(scope.maxNum)){scope.textModel=parseFloat(scope.maxNum);}}
if(scope.minNum!=="min"){if(parseFloat(scope.textModel)<parseFloat(scope.minNum)){scope.textModel=parseFloat(scope.minNum);}}},0);}});scope.numup=function(){$timeout(function(){if(!scope.textModel||isNaN(scope.textModel)){scope.textModel=0;}
scope.textModel=add();if(scope.textModel>parseFloat(scope.maxNum)){scope.textModel=parseFloat(scope.maxNum);}
if(scope.textModel<parseFloat(scope.minNum)){scope.textModel=parseFloat(scope.minNum);}
scope.textModel=changetemp(scope.textModel);},0);};scope.numdown=function(){$timeout(function(){if(!scope.textModel||isNaN(scope.textModel)){scope.textModel=0;}
scope.textModel=sub();if(scope.textModel>parseFloat(scope.maxNum)){scope.textModel=parseFloat(scope.maxNum);}
if(scope.textModel<parseFloat(scope.minNum)){scope.textModel=parseFloat(scope.minNum);}
scope.textModel=changetemp(scope.textModel);},0);};scope.cleanInput=function(){$timeout(function(){scope.textModel="";},0);}
function add(){var tempNum=scope.textModel.toString().split('.');var stepPrec=scope.step.toString().split('.');var result="";var integer;var floatModel,floatStep,float;integer=Number(tempNum[0])+Number(stepPrec[0]);if(scope.precision===0){return integer;}
floatModel=tempNum[1]?tempNum[1].substring(0,scope.precision):'0';floatStep=stepPrec[1]?stepPrec[1].substring(0,scope.precision):'0';for(var i=floatModel.length;i<scope.precision;i++){floatModel+='0';}
for(var j=floatStep.length;j<scope.precision;j++){floatStep+='0';}
if(scope.textModel.toString().indexOf('-')>-1){floatModel="-"+floatModel;}
if(scope.step.toString().indexOf('-')>-1){floatStep="-"+floatStep;}
floatModel=Number(floatModel);floatStep=Number(floatStep);float=floatModel+floatStep;if(float>Math.pow(10,scope.precision)){if(integer.indexOf('-0')===0){integer=integer.replace('-0','0');}
else{integer++;}
float=float.toString().substring(1,float.length);}
integer=integer.toString();for(var k=0;k<scope.precision;k++){integer+='0';}
result=(Number(integer)+Number(float)).toString();result=result.substring(0,result.length-scope.precision)+'.'+result.substring(result.length-scope.precision,result.length);if(result.indexOf('.')===0){result=result.replace('.','0.');}
if(result.indexOf('-.')===0){result=result.replace('-.','-0.');}
return result;}
function sub(){var tempNum=scope.textModel.toString().split('.');var stepPrec=scope.step.toString().split('.');var result="";var integer;var floatModel,floatStep,float;integer=Number(tempNum[0])-Number(stepPrec[0]);if(scope.precision===0){return integer;}
floatModel=tempNum[1]?tempNum[1].substring(0,scope.precision):'0';floatStep=stepPrec[1]?stepPrec[1].substring(0,scope.precision):'0';for(var i=floatModel.length;i<scope.precision;i++){floatModel+='0';}
for(var j=floatStep.length;j<scope.precision;j++){floatStep+='0';}
if(scope.textModel.toString().indexOf('-')>-1){floatModel="-"+floatModel;}
if(scope.step.toString().indexOf('-')>-1){floatStep="-"+floatStep;}
floatModel=Number(floatModel);floatStep=Number(floatStep);float=floatModel-floatStep;float=Math.pow(10,scope.precision)+float;if(float<Math.pow(10,scope.precision)){if(scope.textModel<scope.step&&integer===0){integer="-0";}
else{integer--;}}
else{float=float.toString().substring(1,float.length);}
integer=integer.toString();for(var k=0;k<scope.precision;k++){integer+='0';}
if(integer.indexOf('-0')===0){integer=integer.replace('-0','-1');}
result=(Number(integer)+Number(float)).toString();result=result.substring(0,result.length-scope.precision)+'.'+result.substring(result.length-scope.precision,result.length);if(result.indexOf('.')===0){result=result.replace('.','0.');}
if(result.indexOf('-.')===0){result=result.replace('-.','-0.');}
return result;}
function changetemp(model){var jingdu=parseInt(scope.precision);var isNegative=false;if(jingdu==''||jingdu==null){jingdu=0;}
if(model.toString().indexOf('-')>-1&&parseInt(model)>-1&&parseInt(model)<1){isNegative=true;}
...
var judgeNull=function(){if(!scope.textModel){scope.textErrorMsg=$filter('translate')('setting.required');scope.isErrorClass=true;}}
var judgeLength=function(){if(scope.textLength<scope.textModel.length){scope.textErrorMsg=$filter('translate')('setting.toolong');scope.isErrorClass=true;}}
var judgeContent=function(){var tmpString=scope.textNotAllowed.split("");for(var i=0;i<tmpString.length;i++){if(scope.textModel.indexOf(tmpString[i])>-1){scope.textErrorMsg=$filter('translate')('setting.illegalWord');scope.isErrorClass=true;break;}}}},transclude:true,replace:true}});;'use strict';app.factory('ToggleHelper',['$rootScope',function($rootScope){return{events:{toggle:"clip-two.toggle",toggleByClass:"clip-two.toggleByClass",togglerLinked:"clip-two.linked",toggleableToggled:"clip-two.toggled"},commands:{alternate:"toggle",activate:"on",deactivate:"off"},toggle:function(target,command){if(command==null){command="toggle";}
...
if(jsRequires.modules[m].name&&jsRequires.modules[m].name===name)
return jsRequires.modules[m];return jsRequires.scripts&&jsRequires.scripts[name];}}]};}
return{restrict:'E',template:'<div ng-click="openPopwinFun()" ng-transclude>'+'</div>',scope:{popwinTitle:'=?',popwinIcon:'=?',popwinmodal:'=?',resolve:'@?',templateUrl:'@?',template:'=?',size:'@?',winClass:'@?',backdrop:'=?',onOpened:'=?',onSubmit:'=?',onCancel:'=?',openPopwinFun:'=?',submitText:'@?',cancelText:'@?',},replace:true,transclude:true,controller:function($scope,SweetAlert,$templateCache){function _rdom(){return(((1+Math.random())*0x10000)|0).toString(16).substring(1);}
$scope.ctrl='popwinsCtrl_'+_rdom()+'_'+new Date().getTime();$scope.openPopwinFun=function(){var isShowSubmit=!!$scope.submitText;var isShowCancel=!!$scope.cancelText;var tmls=$scope.template?$scope.template:'    <!-- 表单 -->'+'    <form role="form" class="form-horizontal" name="$popwin_form" novalidate ng-submit="submitForm.submit($popwin_form)">'+
$templateCache.get($scope.templateUrl)+'        <div class="form-group" ng-if="'+(isShowSubmit||isShowCancel)+'" style="margin-top: 50px;margin-bottom: 0px; margin-right: 0px;">'+'            <button type="submit" ng-if="'+isShowSubmit+'" style="min-width: 80px; float: right; margin-left: 0.5em;" class="btn btn-wide btn-primary">'+$scope.submitText+'</button>'+'            <a style="float: right; min-width: 80px;" ng-if="'+isShowCancel+'" class="btn btn-wide btn-o btn-default" ng-click="submitForm.cancel()">'+$scope.cancelText+'</a>'+'        </div>'+'    </form>';tmls='<!-- Head -->'+'<div class="modal-header" style="padding:10px">'+'    <h4 style="margin-bottom: 0px;" ng-class="icon">{{title}}</h4>'+'    <a class="my-pop-win-close" href="#" ng-click="closePop()"><i class="ti-close" tooltip="关闭"></i></a>'+'</div>'+'<!-- Body -->'+'<div class="modal-body form-error">'+
